  ...
  public void save(final File file, final Order order) throws IOException {
    try (BufferedWriter buff = new BufferedWriter(new FileWriter(file))) {
      buff.write(String.format(
          "%d;%d;%d%n", order.getOrderId(), order.getValue(), order.getPizzaList().size()));
      for (Pizza currentPizza : order.getPizzaList()) {
        buff.write(String.format("%s;%s;%s", currentPizza.getPizzaId(), currentPizza.getPrice(),
    ...
    while ((input = reader.readLine()) != null) {
      ...
      if (Arrays.stream(PizzaSize.values()).anyMatch((t) -> t.name().equals(loadedSize))) {
        ...
        throw new WrongOrderFormatException(
            "Pizza Size: " + loadedSize + " not available at this serivce!");
      }
      ...
      while (pizzaToken.hasMoreTokens()) {
        ...
        if (!toppingExists(currentTopping)) {
          throw new WrongOrderFormatException(
              "Topping: " + currentTopping + " not available at this service!");
        }
        try {
